...
then plot individual 1D profiles and full 2D velocity section using PyGMT.
...
- Use the 4 surrounding grid nodes at each depth.
...
-------
1) 1D data:
   output/1D_extract_dist_weighting/{sta}.dat
...
4) Horizontal model-node/station map:
...
OUT_DATA_DIR = Path("output/1D_extract_dist_weighting")
OUT_DATA_DIR.mkdir(parents=True, exist_ok=True)

OUT_FIG_1D_DIR = Path("figures/1D_extract_dist_weighting")
...
# If True, show only model nodes inside the cropped station region
PLOT_ONLY_CROPPED_NODES = True
...
def save_1d_data(sta_name, df_1d):
    out_file = OUT_DATA_DIR / f"{sta_name}.dat"
    arr = df_1d[["dep", "vs", "vp"]].values
    np.savetxt(out_file, arr, fmt="%.6f")
    print(f"[OK] Saved 1D data: {out_file}")
...
def build_section_grid(section_profiles, sta_df_all):
    ...
    for i, sta in enumerate(names_sta):
        ...
        dep_arr = df_1d["dep"].values
        vs_arr = df_1d["vs"].values
...
def plot_full_profiles_pygmt(section_dict, extract_sta=None):
    ...
    # ----------------------------------------------
    # Create topographic colored fill grid
    # ----------------------------------------------
    ...
    fig.basemap(
        ...
        frame=[
            "WSne+tTopography and Vs profile",
...
# ==================================================
# PyGMT horizontal plane plot: velocity + nodes + stations
# ==================================================
def plot_model_nodes_and_stations_pygmt(model, horizontal_nodes, sta_df):
    ...
    # ==================================================
    # Crop region by station locations only
    # ==================================================
    ...
    # ==================================================
    # Model nodes inside cropped region
    # ==================================================
    ...
    # Coastline on top of velocity background
    fig.coast(
        region=region,
        projection="M15c",
    ...
    # ==================================================
    # Model nodes inside station-cropped region
    # ==================================================
    if len(nodes_crop) > 0:
        fig.plot(
            x=nodes_crop["lon"],
    ...
    # ==================================================
    # Station labels every N stations
    # ==================================================
...
def main():
    ...
    # IMPORTANT:
    # PyGMT/GMT modern mode is not safe with multiprocessing.
    # Keep extraction multi-core, but plot figures one-by-one.
    # --------------------------------------------------
